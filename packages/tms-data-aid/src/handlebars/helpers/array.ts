...
helpers.filter = function (array, value, options) {
  ...
  let prop = options.hash && (options.hash.property || options.hash.prop)
  if (prop) {
...
helpers.inArray = function (array, value, options) {
  return utils_hb.value(utils.indexOf(array, value) > -1, this, options)
}
...
helpers.equalsLength = function (value, length, options) {
  ...
  return utils_hb.value(len === length, this, options)
}
...
helpers.length = function (value) {
  if (utils.isObject(value) && !utils_hb.isOptions(value)) {
...
helpers.sortBy = function (array, prop, options) {
  ...
  if (!utils.isString(prop) && typeof prop !== 'function') {
    return array.sort()
  }
...
helpers.withFirst = function (array, idx, options) {
  ...
  if (!utils.isUndefined(idx)) {
    idx = parseFloat(utils_hb.result(idx))
  }
