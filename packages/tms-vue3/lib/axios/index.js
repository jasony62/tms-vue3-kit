    ...
    rules.forEach((rule) => {
        ...
        let onFulfilled = async function (config) {
            ...
            if (requestParams) {
                ...
                requestParams.forEach((v, k) => typeof v === 'function'
            ...
            if (requestHeaders) {
                ...
                requestHeaders.forEach((v, k) => typeof v === 'function'
...
function useResponseInterceptor(tmsAxios, rules) {
    ...
    onRejectedRules = rules.filter((rule) => typeof rule.onResponseRejected === 'function');
    /*重发请求*/
        ...
        tmsAxios.interceptors.response.use((res) => {
            if (res.data.code !== 0) {
                ...
                if (undefined === res.config.headers['TmsAxios-Retry']) {
                    ...
                    let retryPromises = retryRules.map((rule) => rule.onRetryAttempt(res, rule));
                    return Promise.all(retryPromises).then((isRetry) => isRetry.includes(true) ? tmsAxios.request(res.config) : res);
                }
            }
            return res;
        });
    ...
    if (onRejectedRules.length) {
        onRejectedRules.forEach((rule) => {
            tmsAxios.interceptors.response.use((res) => res, (err) => {
