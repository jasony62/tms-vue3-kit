...
function splice(target, start, deleteCount, ...items) {
  if (!Array.isArray(target))
    return [];
  const arr = target[ARR_ASSOC];
  arr.splice(start, deleteCount, items.map((item, index) => ({ key: start + index, val: item })));
...
function findKeyIndex(target, searchKey) {
  ...
  if (arr) {
    ...
    if (index === -1 && Number.isInteger(searchKey)) {
      if (searchKey >= 0 && searchKey < arr.length)
        index = searchKey;
    }
    return index;
  }
...
function move(handler, indexOrProp, step) {
  ...
  if (isArray) {
    ...
    const fnDel = typeof customHandler.deleteProperty === "function" ? (key) => customHandler.deleteProperty(target, key) : (key) => delete target[key];
    affected.forEach(({ key }) => fnDel(key));
    const fnSet = typeof customHandler.defineProperty === "function" ? (key, val) => customHandler.defineProperty(target, key, val) : (key, val) => target[key] = val;
...
class AssocHandler {
  ...
  get(target, property) {
    if (property === "__target__") {
      ...
      return function(search, from = 0) {
        return findIndexKey(target, search);
      };
    ...
    if (!Array.isArray(target)) {
      ...
      if (!isNaN(parseInt(property))) {
        if (property < 0 && property >= arr.length)
          return void 0;
...
function install(app) {
  app.component("tms-object-input", {
    ...
    data() {
      const lines = typeof this.value === "object" ? new Objarr(this.value, {
    ...
    methods: {
      createLine(createElement, line, index, key) {
        const buttonNodes = [];
        let { remove, moveup, movedown } = components.button;
        buttonNodes.push(createElement(remove.component, __spreadValues({
          onClick: (evt) => this.remove(evt, line, index, key)
        }, remove.options), this.$slots.remove ? this.$slots.remove() : "\u5220\u9664"));
        if (index > 0) {
          buttonNodes.push(createElement(moveup.component, __spreadValues({
            onClick: (evt) => this.moveUp(evt, line, index)
          }, moveup.options), this.$slots.moveup ? this.$slots.moveup() : "\u4E0A\u79FB"));
        }
        if (index < this.lines.length - 1) {
          buttonNodes.push(createElement(movedown.component, __spreadValues({
            onClick: (evt) => this.moveDown(evt, line, index)
          }, movedown.options), this.$slots.movedown ? this.$slots.movedown() : "\u4E0B\u79FB"));
        }
        let { layout } = components;
        const lineChildNodes = [];
        if (this.isInputObject) {
          const lineKeyNode = createElement(layout["line-key"].component, __spreadValues({
            domProps: {
              value: key
            },
            props: {
          ...
          const lineIndexNode = createElement(layout["line-index"].component, __spreadValues({}, layout["line-index"].options), index);
          lineChildNodes.push(lineIndexNode);
        }
        const lineSlotNode = createElement(layout["line-slot"].component, __spreadValues({}, layout["line-slot"].options), [
          this.$slots.default ? this.$slots.default({ line }) : typeof this.slotRender === "function" ? this.slotRender(createElement, { line }) : line
        ]);
        lineChildNodes.push(lineSlotNode);
        const lineButtonsNode = createElement(layout["line-buttons"].component, __spreadValues({}, layout["line-buttons"].options), buttonNodes);
        lineChildNodes.push(lineButtonsNode);
        return Render.layered(createElement, layout.line.component, layout.line.options, lineChildNodes);
      },
      createBottom(createElement) {
        let { bottom } = components.layout;
        let { add, empty } = components.button;
        return createElement(bottom.component, [
          createElement(add.component, __spreadValues({
            onClick: this.emitAdd
          }, add.options), this.$slots.add ? this.$slots.add() : "\u6DFB\u52A0"),
          createElement(empty.component, __spreadValues({
            onClick: this.empty
          }, empty.options), this.$slots.empty ? this.$slots.empty() : "\u6E05\u7A7A")
        ]);
      },
      emitAdd() {
    ...
    render() {
      ...
      const nodes = [];
      let { layout } = components;
      let linesNode = h(layout.lines.component, __spreadValues({}, layout.lines.options), this.lines.map((line, index) => this.createLine(h, line, index, this.lines.findIndexKey(index))));
      nodes.push(linesNode);
      nodes.push(this.createBottom(h));
      return h(layout.root.component, __spreadValues({}, layout.root.options), nodes);
    }
  });
}
